...
class RealEstateScraper(scrapy.Spider):
    ...
    def start_requests(self):
      ...
      with open(filename, 'w') as f:
        columns = [
          'url', 'id', 'title', 'description', 'createdAt', 'modifiedAt',
          'images', 'floorPlans', 'target', 'characteristics', 'latitude', 'longitude'
        ]
    ...
    def parse_links(self, response):
      ...
      try:
        ...
        total_pages = int(json_data['props']['pageProps']['tracking']['listing']['page_count'])-2
    ...
    def parse_listing(self, response):
      ...
      # Extract JSON data
      json_data = json.loads(response.css('script[id="__NEXT_DATA__"]::text').get())['props']['pageProps']
      
      # CSV entry
      features = {
        'url': url,
        'id': json_data['ad']['id'],
        'title': json_data['ad']['title'],
        'description': json_data['ad']['description'].replace('\n', '').replace('\r', ''),
        'createdAt': json_data['ad']['createdAt'],
        'modifiedAt': json_data['ad']['modifiedAt'],
        'images': [i['large'] for i in json_data['ad']['images']],
        'floorPlans': json_data['ad']['floorPlans'],
        'target': json_data['ad']['target'],
        'characteristics': json_data['ad']['characteristics'],
        'latitude': json_data['ad']['location']['coordinates']['latitude'],
        'longitude': json_data['ad']['location']['coordinates']['longitude']
      }
